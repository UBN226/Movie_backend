...
@app.get(
    "/movies/{movie_id}", # url de l'endpoint
    ...
    response_model=schemas.MovieDetailed, # permet de valider et de formater la réponse json avec le schema Pydantic MovieDetailed
    tags=["films"],
)
...
def list_movies(
    ...
    db: Session = Depends(get_db) # connexion temporaire à la bd
):
...
def read_rating(
    ...
    if rating is None:
        raise HTTPException(status_code=404, detail=f"Aucune évaluation trouvée pour l'utilisateur {user_id} et le film {movie_id}" )
    return rating
